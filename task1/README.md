...
Я добавил комментарии на основные структуры и сделал юнит тесты на сервисный слой и воркера, а так же интеграционные тесты.
Хотел добавить тесты на другие модули, но там очень геморно добавлять моки и симулировать запуск баз, поэтому я сосредоточился на проверке логики работы.
Увидел, что в одном селекте context.Done стоит в конце, из-за чего цикл может уйти в continue, а не в return из-за порядка условий, поменял их местами.

Не очень понял, про метрики, пинг для редиса я добавил для проверки его работоспособности, метрики это прикручивание прометеуса, а в требованиях этого нет.